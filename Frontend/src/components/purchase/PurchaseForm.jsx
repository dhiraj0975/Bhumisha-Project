...
const to24h = (hhmm = "00:00", ampm = "PM") => {
  let [hh, mm] = (hhmm || "00:00").split(":").map((x) => Number(x || 0));
  if (ampm === "AM") { if (hh === 12) hh = 0; } else { if (hh !== 12) hh = hh + 12; }
  return `${String(hh).padStart(2, "0")}:${String(mm).padStart(2, "0")}:00`;
};
    ...
    <form onSubmit={onSubmit} className="p-3">
      ...
      <div className="grid grid-cols-6 gap-3 border p-3 rounded">
        ...
        <div className="flex flex-col">
          <label className="text-xs">BILL TIME</label>
          <div className="flex gap-1">
            <input type="time" name="bill_time" value={header.bill_time || "00:00"} onChange={onHeader} className="border rounded p-1" />
          </div>
        </div>
        <div className="flex flex-col">
          <label className="text-xs">Vendor</label>
          <select className="border rounded p-1" name="vendor_id" value={header.vendor_id} onChange={onVendorChange}>
            <option value="">Select</option>
            {vendors.map((v) => (
      ...
      <div className="overflow-auto">
        <table className="w-full text-sm border">
          <thead className="bg-green-700 text-white">
            <tr>
              {["SI","Item Name","HSNCode","Available","QTY","Rate","Amount","Disc %","Per Qty Disc","Total Disc","GST%","GST Amt","FinalAmt","Actions"].map((h, idx) => (
                <th key={`${h}-${idx}`} className="border px-2 py-1 text-left">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {rows.map((r, i) => {
              ...
              return (
                <tr key={i} className="odd:bg-white even:bg-gray-50">
                  ...
                  <td className="border px-2 py-1">
                    <div className="flex gap-1">
                      ...
                      >
                        <option value="">Select</option>
                        {products.map((p) => (
                          <option key={p.id} value={String(p.id)}>{p.product_name}</option>
                        ))}
                      </select>
                    </div>
                  </td>
                  ...
                  <td className="border px-2 py-1">
                    <input readOnly className="border cursor-not-allowed bg-gray-100 rounded p-1 w-24" value={r.hsn_code} onChange={(e) => onRow(i, "hsn_code", e.target.value)} />
                  </td>
                  ...
                  <td className="border px-2 py-1">
                    <input type="number" className="border rounded p-1 w-20" value={r.size} onChange={(e) => onRow(i, "size", e.target.value)} />
                  </td>
                  ...
                  <td className="border px-2 py-1">
                    <input type="number" className="border rounded p-1 w-16" value={r.d1_percent} onChange={(e) => onRow(i, "d1_percent", e.target.value)} />
                  </td>
                  ...
                  <td className="border px-2 py-1">
                    <input type="number" className="border rounded p-1 w-16" value={r.gst_percent} onChange={(e) => onRow(i, "gst_percent", e.target.value)} />
                  </td>
                  ...
                  <td className="border px-2 py-1 text-center">
                    <button type="button" className="text-red-600 active:scale-95" onClick={() => setRows((p) => p.filter((_, idx) => idx !== i))}>
                      ❌
                    </button>
                  </td>
                </tr>
      ...
      <div className="flex gap-2 mt-3">
        <button type="button" onClick={() => setRows((p) => [...p, { product_id: "", item_name: "", hsn_code: "", available: 0, size: 1, rate: 0, d1_percent: 0, gst_percent: 0 }])} className="px-4 py-2 bg-blue-600 action:scale-95 text-white rounded">
          Add Item
        </button>
        <button type="submit" disabled={loading || !isFormValid} className={`px-6 py-2 active:scale-95 rounded text-white bg-green-700 transition-opacity duration-200 ${loading || !isFormValid ? "opacity-50 cursor-not-allowed" : "opacity-100 cursor-pointer"}`}>
          {loading ? (isEditMode ? "Updating..." : "Saving...") : isEditMode ? "Update" : "Save Purchase"}
        </button>
      </div>
    </form>
