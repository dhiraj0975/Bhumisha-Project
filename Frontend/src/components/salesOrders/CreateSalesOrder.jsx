...
export default function CreateSalesOrder({ so = null, onSaved }) {
  ...
  return (
<form onSubmit={onSubmit} className="bg-white shadow rounded-xl p-5 md:p-6">
  {/* Title */}
  <div className="flex items-center justify-between mb-3">
    <h3 className="text-lg md:text-xl font-semibold">{isEditMode ? "Edit Sales Order" : "Create Sales Order"}</h3>
      ...
      <div className="bg-white border rounded-lg shadow-sm p-4">
        ...
        <div className="pt-2 mt-2 border-t space-y-1">
          ...
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="text-gray-800">{form.place_of_supply || "-"}</span>
          </div>
        </div>
      </div>
    </div>
  ...
  <div className="mt-2">
    ...
    {form.items.map((it, i) => {
      ...
      return (
        <div key={i} className="mb-3 border rounded-lg p-3 bg-white shadow-sm">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-2">
            ...
            <input className="border rounded p-2" name="qty" type="number" min={0} step="0.01" inputMode="decimal" value={it.qty} onChange={(e) => onItem(i, e)} placeholder="Qty" />
            <input className="border rounded p-2" name="rate" type="number" min={0} step="0.01" inputMode="decimal" value={it.rate} onChange={(e) => onItem(i, e)} placeholder="Rate" />
            <input className="border rounded p-2" name="d1_percent" type="number" min={0} step="0.01" inputMode="decimal" value={it.d1_percent} onChange={(e) => onItem(i, e)} placeholder="Disc%/Unit" />
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-2 mt-2 items-center">
            <input className="border rounded p-2" name="gst_percent" type="number" min={0} step="0.01" inputMode="decimal" value={it.gst_percent} onChange={(e) => onItem(i, e)} placeholder="GST%" />
            <div className="text-xs text-gray-600">Amount: <span className="font-medium text-gray-800">{fx(c.amount)}</span></div>
            <div className="text-xs text-gray-600">Disc Total: <span className="font-medium text-gray-800">{fx(c.discTotal)}</span></div>
            <div className="text-xs text-gray-600">Taxable: <span className="font-medium text-gray-800">{fx(c.taxable)}</span></div>
            <div className="text-right">
              <button type="button" className="px-3 py-1 bg-red-500 text-white rounded hover:bg-red-600 active:scale-95" onClick={() => removeItem(i)}>
                Remove
              </button>
            </div>
          </div>
    ...
    <button type="button" onClick={addItem} className="mt-1 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 active:scale-95">
      + Add Item
    </button>
  </div>
  ...
  <div className="pt-3 flex gap-3 justify-end">
    ...
    <button
      ...
      className={`px-6 py-2 rounded text-white ${!isValid || loading ? "bg-green-700/50 cursor-not-allowed" : "bg-green-700 hover:bg-green-800 active:scale-95"}`}
    >
