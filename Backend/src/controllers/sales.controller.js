...
const SalesController = {
  async createSale(req, res) {
    try {
      const { customer_id, bill_no, bill_date, items, status = 'Active', payment_status = 'Unpaid', payment_method = 'Cash', remarks = null } = req.body;
      if (!customer_id || !bill_date || !Array.isArray(items) || !items.length) {
        return res.status(400).json({ error: 'Customer, bill_date and items[] required' });
      }
      const result = await Sales.create({ customer_id, bill_no, bill_date, status, payment_status, payment_method, remarks, items });
      return res.status(201).json({
        message: 'Sale created successfully',
        id: result.id,
        bill_no: result.bill_no,
